    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        if (label.equalsIgnoreCase("tpahere")) {
            if (sender instanceof Player) {
                ...
                Bukkit.getPlayer(args[0]).sendMessage("§7" + sender.getName() + " would like you to teleport to them.\n  §7Accept with §b/tpaccept\n  §7Deny with §b/tpdeny");

                // you sent player blah a tpa request
                sender.sendMessage("§7You sent " + Bukkit.getPlayer(args[0]).getName() + " a tpahere request.");

                // add them to the maps
                TpahereCommand.requesting.put(((Player) sender).getUniqueId(), Bukkit.getPlayer(args[0]).getUniqueId());
